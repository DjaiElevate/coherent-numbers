"""SPY daily data loader for the harmonic calendar pre-registered study.

Pre-registration anchor: commit 0cc8e6a
Locked protocol:         docs/harmonic_calendar_design_memo_v0.3.1.md

RUNTIME DATA SOURCE
===================
The canonical runtime loader (load_spy) reads from a FROZEN LOCAL CSV FILE,
not from a live network request. Yahoo Finance is the acquisition source only.
The frozen CSV is produced once by acquire_spy_from_yahoo() and is thereafter
treated as immutable. Its SHA256 hash is embedded in the filename so that any
subsequent modification is immediately detectable at load time.
...
Adjusted price field: Adjusted close — adjusted for all splits and dividend
                      reinvestment throughout SPY's history. Unadjusted close
                      is NOT used; it would introduce systematic return errors
                      on every ex-dividend date and overstate long-run drawdowns.
Start date:           1993-01-29 (SPY's first trading day; earliest available)
End / cutoff:         2024-12-31 — the last complete calendar year before the
                      pre-registration timestamp (2026-05-11). Data from
                      2025-01-01 onward is excluded unconditionally. The
                      year-end boundary avoids selection bias that would arise
                      from a mid-year cutoff.
Frozen CSV location:  data/raw/spy_yahoo_v8_19930129_20241231_{sha256}.csv
                      The SHA256 is computed over the full CSV byte contents
                      and embedded in the filename. load_spy() verifies the
...
TRAIN / HOLDOUT SPLIT (locked in memo v0.3.1, section "Train / Holdout Split")
===============================================================================
Training:  dates in [START_DATE, HOLDOUT_START)   — approx 1993-01-29 to 2014-12-31
...
START_DATE         = datetime.date(1993, 1, 29)   # SPY first trading day
CUTOFF_DATE        = datetime.date(2024, 12, 31)  # last day before pre-registration
HOLDOUT_START      = datetime.date(2015, 1, 1)    # final 10 calendar years: 2015–2024
FILL_LIMIT         = 5                            # max consecutive NaN to forward-fill

# Filename pattern produced by acquire_spy_from_yahoo.
# {sha256} is the lowercase hexadecimal SHA256 of the CSV file contents.
FROZEN_CSV_STEM_FORMAT = "spy_yahoo_v8_19930129_20241231_{sha256}.csv"
...
_YF_URL     = "https://query1.finance.yahoo.com/v8/finance/chart/SPY"
_YF_HEADERS = {"User-Agent": "Mozilla/5.0 (harmonic-calendar-research/0.3.1)"}


# ── Cleaning ──────────────────────────────────────────────────────────────────

def clean_spy_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Input:  DataFrame with a DatetimeIndex (or datetime-convertible index) and
            an [adj_close] column. Accepted from load_spy (reads frozen CSV) or
...
def make_train_holdout_split(
    ...
    """Split a clean SPY DataFrame into training and holdout partitions.

    Training:  date in [START_DATE, HOLDOUT_START)   — before 2015-01-01
    Holdout:   date in [HOLDOUT_START, CUTOFF_DATE]  — 2015-01-01 .. 2024-12-31

    Rules enforced:
    - No temporal overlap: every training date precedes every holdout date.
    - No data leakage: the split key HOLDOUT_START was fixed before any data
      was observed.
    ...
    if "date" not in df.columns:
        raise ValueError("DataFrame must have a 'date' column (output of clean_spy_df)")
...
def load_spy(csv_path: str) -> pd.DataFrame:
    """Canonical runtime loader: read the frozen SPY CSV and return a clean DataFrame.
    ...
    Args:
        csv_path: Absolute or relative path to the frozen CSV file produced by
                  acquire_spy_from_yahoo(). Expected filename format:
                  spy_yahoo_v8_19930129_20241231_{sha256}.csv
    ...
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            "Frozen SPY CSV not found at {!r}.\n"
            "Run acquire_spy_from_yahoo() once to produce the frozen CSV,\n"
            "then pass the returned path to load_spy().".format(csv_path)
        )
    _verify_csv_hash_from_filename(csv_path)
    raw = pd.read_csv(csv_path, parse_dates=["date"], index_col="date")
    return clean_spy_df(raw)


# ── Acquisition (run ONCE, explicitly, never from analysis code) ──────────────

def _download_yahoo_adjclose() -> pd.DataFrame:
    """Private helper: fetch SPY adjclose from Yahoo Finance v8 API.

    Returns a DataFrame with DatetimeIndex (tz-naive) and [adj_close] column.
    Only called from acquire_spy_from_yahoo(); never called by the canonical loader.
    """
...
def acquire_spy_from_yahoo(output_dir: str = "data/raw") -> str:
    """ACQUISITION FUNCTION — RUN ONCE.
    ...
    loader. The canonical loader (load_spy) reads from the local frozen CSV
    produced by this function. Do not call this function from analysis code.

    Steps performed:
      1. Fetch SPY daily adjusted close from Yahoo Finance v8 chart API.
      ...
      3. Write raw adjusted-close data to CSV (no log-return computation here;
         clean_spy_df handles that at load time).
    ...
    print("SPY acquisition complete.")
