"""Candidate C parameterized annual-sector lens (design memo 401ce45, sections 7.1 / 7.2).
...
in tests/test_candidate_c_lens.py asserts the absence of that operator from
this file. The gate scans non-comment code and string text alike, so that
character is kept out of docstrings too; the word "modulo" is used instead.
...
there is deliberately no module-level bucket-count constant.
"""
...
def _anchor_date_in_year(year: int, anchor_doy: int) -> date:
    ...
    Following the inherited Candidate B convention, date(year, 1, 1) plus
    (anchor_doy - 1) days resolves leap days through standard datetime
    arithmetic; DOY 60 in a leap year lands on Feb 29.
    """
...
def enumerate_bucket_counts() -> Tuple[int, int]:
    """Return the locked bucket-count pair for Candidate C: (12, 10)."""
    return (12, 10)
